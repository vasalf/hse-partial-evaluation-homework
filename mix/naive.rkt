...
(define mix-naïve
  `((import (initial-block    . ,initial-block)
    ...
    (loopb        (:= pp (caar pending))
                  (:= vs (cadar pending))
                  (:= pending (cdr pending))
                  (:= marked (cons (block-name pp vs live-variables) marked))
    ...
    (dassign          (:= x (cadr command))
                      (:= e (caddr command))
                      (:= code (extend-bb code `(:= ,x ,(reduce e vs division (import-stmt program)))))
                      (goto loopbb))
    ...
    (dif              (:= ift (caddr command))
                      ...
                      (:= e (reduce (cadr command) vs division (import-stmt program)))
                      (:= code (extend-bb code `(if ,e ,nift ,niff)))
                      (goto loopbb))
    (creturn          (:= code (extend-bb code `(return ,(reduce (cadr command) vs division (import-stmt program)))))
                      (goto loopbb))
